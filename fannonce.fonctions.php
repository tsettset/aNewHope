...
function insertionAnnonce($check, $photos){//Envoi vers la bdd - retourne l'id_annonce si reussite, 0 sinon
  ...
  try{
    ...
    $req_insert = $bdd->prepare(
      "INSERT INTO annonce (titre, description_courte, description_longue, prix, pays, ville, cp, date_enregistrement, membre_id, categorie_id)
      VALUES (:titre, :description_courte, :description_longue, :prix, :pays, :ville, :cp, NOW(), 1, :categorie_id);");
      ...
      $req_insert->bindValue(':categorie_id', $check['categorie'], PDO::PARAM_INT);
      $req_insert->execute();
      if (isset($id_photos)){
